...
test('dock and panel are separate accessible surfaces with deterministic closing', () => {
  ...
  assert.match(dock, /:class="\['home-service-panel', `home-service-panel--\$\{props\.activePanel\}`\]"/)
...
test('panel overlays desktop, stays hidden on mobile, and respects reduced motion', () => {
  ...
  assert.match(styles, /\.home-service-panel\s*\{[^}]*position:\s*fixed;[^}]*bottom:\s*1rem;/s)
  ...
  assert.match(guide, /\.home-service-guide\s*\{[^}]*grid-template-columns:\s*minmax\(0, 7fr\) minmax\(15rem, 5fr\);/s)
