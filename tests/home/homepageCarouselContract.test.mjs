...
test('Homepage owns a separate accessible Swiper with bounded autoplay behavior', () => {
  ...
  assert.match(carousel, /:slides-per-view="hasMultiple \? 1 : 1"/); assert.match(carousel, /delay: 5000/); assert.match(carousel, /disableOnInteraction: false/)
  assert.match(carousel, /maxConfiguredSlidesPerView = Math\.max\(1, 1\.55, 2\.05\)/)
  ...
  assert.match(carousel, /641: \{ slidesPerView: 1\.55, spaceBetween: 18 \}/)
  assert.match(carousel, /960: \{ slidesPerView: 2\.05, spaceBetween: 0 \}/)
  assert.match(carousel, /@media\(max-width:640px\).*swiper-slide-prev.*opacity:0;visibility:hidden;pointer-events:none/s)
