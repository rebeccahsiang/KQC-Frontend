...
test('Homepage owns a separate accessible Swiper with bounded autoplay behavior', () => {
  ...
  assert.match(carousel, /maxConfiguredSlidesPerView = Math\.max\(1\.08, 1\.55, 2\.05\)/)
  assert.match(carousel, /canLoop = computed\(\(\) => visibleImages\.value\.length > maxConfiguredSlidesPerView\)/)
  assert.match(carousel, /:loop="canLoop"/); assert.match(carousel, /:navigation="hasMultiple"/); assert.match(carousel, /:pagination="hasMultiple \? \{ clickable: true \} : false"/)
