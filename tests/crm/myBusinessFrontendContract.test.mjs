...
test('Calendar is lightweight, DateTime-based, non-polling, and localization stays frontend-owned', () => {
  const source = read('src/views/admin/crm/MyBusinessView.vue')
  const calendarLogic = source.slice(source.indexOf('const calendarRange'), source.indexOf('const loadCustomers'))
  const calendarTemplate = source.slice(source.indexOf('<article class="surface-card calendar-section">'), source.indexOf('<article class="surface-card data-section prospect-section">'))
  assert.match(calendarLogic, /from\.toISOString\(\)/)
  assert.match(calendarLogic, /to\.toISOString\(\)/)
  assert.match(calendarLogic, /7 \* 86400000/)
  assert.match(calendarLogic, /crmApi\.myBusinessCalendar\(range\.from, range\.to\)/)
  assert.match(calendarTemplate, /activityLabel\(item\.activityType\)/)
  assert.doesNotMatch(calendarLogic + calendarTemplate, /FullCalendar|setInterval|setTimeout|heartbeat|polling/i)
  assert.doesNotMatch(source, /(?:setInterval|setTimeout)\s*\(\s*(?:loadCalendar|\(\)\s*=>\s*(?:void\s+)?loadCalendar\(\)|async\s*\(\)\s*=>\s*(?:await\s+)?loadCalendar\(\))/)
