...
test('BusinessCase source remains disabled and deferred without fake IDs or manual ObjectId entry', () => {
  assert.match(view, /來源業務案件 \*/)
  assert.match(view, /select[^>]*disabled[^>]*aria-describedby="business-case-help"/)
  assert.match(view, /待業務管理案件資料來源開放後自動帶入/)
  assert.match(view, /!form\.businessCaseId/)
