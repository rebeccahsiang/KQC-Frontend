...
const maxConfiguredSlidesPerView = Math.max(1, 1.55, 2.05)
...
const carouselBreakpoints = {
  641: { slidesPerView: 1.55, spaceBetween: 18 },
...
<template>
  ...
  <section v-if="loading || visibleImages.length" class="home-carousel-section" aria-label="首頁輪播圖片" :aria-busy="loading">
    ...
    <Swiper v-else class="home-carousel-swiper" :modules="modules" :effect="hasMultiple ? 'coverflow' : 'slide'" :centered-slides="hasMultiple" :slides-per-view="hasMultiple ? 1 : 1" :space-between="hasMultiple ? 12 : 0" :breakpoints="hasMultiple ? carouselBreakpoints : undefined" :coverflow-effect="coverflowEffect" :loop="canLoop" :navigation="hasMultiple" :pagination="hasMultiple ? { clickable: true } : false" :autoplay="hasMultiple && !reducedMotion ? { delay: 5000, disableOnInteraction: false, pauseOnMouseEnter: true } : false" :watch-overflow="true" :grab-cursor="hasMultiple" :keyboard="{ enabled: true }">
...
@media(max-width:640px){.home-carousel-section{width:min(100% - 1.25rem,90rem);padding-block:1.75rem}.home-carousel-swiper :deep(.swiper-slide-prev),.home-carousel-swiper :deep(.swiper-slide-next){opacity:0;visibility:hidden;pointer-events:none}.home-carousel-slide__action,.home-carousel-swiper :deep(.swiper-slide-prev) .home-carousel-slide__action,.home-carousel-swiper :deep(.swiper-slide-next) .home-carousel-slide__action,.home-carousel-swiper :deep(.swiper-slide-active) .home-carousel-slide__action,.home-carousel-swiper :deep(.swiper-slide-active) .home-carousel-slide__action:hover{transform:none}.home-carousel-swiper :deep(.swiper-button-prev),.home-carousel-swiper :deep(.swiper-button-next){display:none}.home-carousel-swiper :deep(.swiper-pagination){bottom:.35rem}}
